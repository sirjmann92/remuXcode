...
| **Fix Container Mismatch** | Video always encodes to a real Matroska file internally, even when the source (and tracked filename) is `.mp4` — the final write does not re-mux, so an `.mp4` that Video re-encoded ends up holding genuine Matroska content under an `.mp4` name. When enabled, remuXcode renames the file to match its real container (and triggers a Sonarr/Radarr refresh so they pick up the corrected name) whenever Video was the last phase to run in a job. **Off by default**: Sonarr/Radarr can only reconcile the rename by deleting and recreating the underlying moviefile/episodefile record, which permanently clears that record's Scene Name and Original File Path metadata (used by some subtitle tools) — this cannot be recovered afterward, by remuXcode or any other means. Custom format score and release group are preserved. A one-time library-wide sweep for files already affected by this (from before the setting existed) is available via `scripts/fix_container_mismatches.py` (dry-run by default; see the script's own `--help`). | `false` |
