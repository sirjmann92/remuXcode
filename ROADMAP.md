...
- **Settings** — all settings configurable from the UI: audio, video, cleanup, language detection, Sonarr/Radarr connections, workers, job retention, hardware acceleration, cover art stripping, **container/extension mismatch correction** (opt-in; renames a file to match its real container when Video's re-encode leaves it under its old extension, then triggers a Sonarr/Radarr refresh — see [settings.md](docs/settings.md))
...
- ✅ **FIXED — Unreadable subtitle streams crash every worker** — Some subtitle streams (seen in WebVTT tracks from certain streaming-service WEBDL rips) have no codec FFmpeg can identify at all — distinct from a codec FFmpeg can decode but the Matroska muxer refuses to write (`mov_text`, `eia_608`, already handled). Mapping one into the output made the Matroska muxer refuse to write the header at all, killing every stream in the file ("Subtitle codec 0 is not supported", "Nothing was written into output file"). Every worker now detects and drops these streams unconditionally, in every phase, regardless of target container — with output-position tracking fixed so dropping a stream correctly shifts later `-c:s:N`/`-metadata:s:s:N` targeting instead of crashing against the now-shorter output stream list
- ✅ **FIXED — Container/extension mismatch after Video re-encode** — Video always encodes to a real Matroska temp file internally regardless of the source container, then moves those bytes onto the tracked path verbatim (no re-mux) — so an `.mp4` whose last processing phase was Video ended up holding genuine Matroska content under an `.mp4` name (mp4 in, mp4 out, despite different bytes). Added an opt-in **Fix Container Mismatch** setting (default off, see [settings.md](docs/settings.md)) that renames the file to match its real container and triggers a Sonarr/Radarr refresh, gated precisely on Video having succeeded and been the final phase (nothing ran after it that would have re-muxed to the original container). Off by default and clearly labeled: Sonarr/Radarr can only pick up the rename by deleting and recreating the underlying moviefile/episodefile record, which permanently clears that record's Scene Name and Original File Path — confirmed via extensive live testing against a production Radarr instance (including checking Radarr's own GitHub issue tracker for a way around it) that this is unrecoverable via any API or mechanism, not just an oversight. Custom format score and release group are preserved. `scripts/fix_container_mismatches.py` does a one-time, dry-run-by-default sweep of the whole library for files already affected by this from before the setting existed