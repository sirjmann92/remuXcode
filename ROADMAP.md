...
- **Optional Plex auth** — allow signing in to the remuXcode UI with a Plex account (OAuth via plex.tv) as an alternative/companion to the API key, useful for multi-user households where sharing the raw API key isn't desirable
