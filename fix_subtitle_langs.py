...
Language tags are inferred from, in priority order:
  1. A MediaInfo ``.xml`` sidecar next to the processed file (if present)
  2. An unprocessed sibling episode in the same show directory
  3. The ``--languages`` fallback (manual specification)
...
# ---------------------------------------------------------------------------
# MediaInfo XML sidecar
# ---------------------------------------------------------------------------

_MI_NS = "{https://mediaarea.net/mediainfo}"


def _read_xml_sidecar(mkv: Path) -> list[dict] | None:
    """Parse a MediaInfo .xml sidecar for per-track subtitle language/title data.

    Returns a list of dicts with 'language' and 'title' keys (one per Text
    track, in order), or None if no usable sidecar is found.
    """
    xml_path = mkv.with_suffix(".xml")
    ...
    root = tree.getroot()
    subs = []
    for track in root.iter(f"{_MI_NS}track"):
        if track.get("type") != "Text":
            continue
        # Prefer 3-letter ISO 639-2/B code (Language_String3), fall back to 2-letter
        lang = (
            track.findtext(f"{_MI_NS}Language_String3")
            or track.findtext(f"{_MI_NS}Language_String2")
            or track.findtext(f"{_MI_NS}Language")
        )
        title = track.findtext(f"{_MI_NS}Title")
        if lang and lang not in _NULL_LANGS:
            subs.append({"language": lang, "title": title})
    return subs or None
...
def process_show(
    ...
    for mkv, subs in affected:
        ...
        if fallback_langs:
            ...
            # 1. Try a MediaInfo .xml sidecar next to this file
            xml_subs = _read_xml_sidecar(mkv)
            if xml_subs is not None and len(xml_subs) >= null_count:
                log.info("    Reference: %s (MediaInfo XML sidecar)", mkv.stem + ".xml")
                ref_iter = iter(xml_subs)
                for sub in subs:
                    if sub["language"] in _NULL_LANGS:
                ...
                # 2. Find an unprocessed sibling MKV
        ...
        if ok:
            log.info("    %s", "Would fix  (dry-run)" if dry_run else "Fixed")
            fixed += 1
        else:
            log.error("    Failed")
            skipped += 1
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        metavar="LANG[,LANG...]",
        help=(
            "Comma-separated language codes to apply to null-language subtitle streams in order "
