...
async function queueMovie(movie: BrowseMovie) {
  ...
  try {
    await convertFile(movie.path, 'full', movie.poster, 'movie');
  } catch {
    // ignore
  } finally {
...
async function queueAllFiltered() {
  ...
  try {
    for (const m of items) {
      await convertFile(m.path, 'full', m.poster, 'movie');
    }
  } catch {
    // ignore
  } finally {
      ...
      {#each filtered as movie (movie.id)}
        ...
        >
          ...
          <div class="aspect-[2/3] bg-base-300 relative overflow-hidden">
            ...
            {/if}
            <!-- Radarr link -->
            {#if config?.radarr.configured && movie.tmdb_id}
              <a
                href="{config.radarr.url}/movie/{movie.tmdb_id}"
                target="_blank"
                rel="noopener noreferrer"
                class="absolute bottom-2 right-9 btn btn-circle btn-xs btn-ghost bg-black/40 hover:bg-black/70 text-white pointer-events-auto"
      ...
      {#if needsWork(detailMovie)}
        ...
        <div class="space-y-3">
          ...
          {#if detailMovie.needs_cleanup && config}
            ...
            {@const keepAudio = (detailMovie.is_anime && config.cleanup.anime_keep_original_audio) ? detailMovie.audio_languages : (!detailMovie.is_anime && config.cleanup.keep_original_audio) ? detailMovie.audio_languages : keptTracks(detailMovie.audio_languages, config)}
        ...
        <div class="modal-action">
          ...
          <button
            class="btn btn-ghost btn-sm"
            onclick={() => { customEncodePath = { path: detailMovie!.path, poster: detailMovie!.poster }; detailMovie = null; }}
          >
            Custom Encode
          </button>
          {#if config?.radarr?.configured}
            <a
              href="{config.radarr.url}/movie/{detailMovie.tmdb_id}"
              target="_blank"
              rel="noopener noreferrer"
...
{#if customEncodePath}
  <ConvertOptionsModal
    paths={customEncodePath.path}
    poster_url={customEncodePath.poster}
    media_type="movie"
    onclose={() => (customEncodePath = null)}
  />
{/if}
