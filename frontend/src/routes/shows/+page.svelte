...
let analyzePath: { path: string; sonarr_episode_file_id?: number } | null = $state(null);
        ...
        <div class="flex-1 min-w-0 space-y-2">
          ...
          <div class="flex gap-2 mt-2">
            ...
            <button
              ...
              onclick={rescanShow}
              disabled={rescanning}
              title="Reload this show's episode details from Sonarr and our analysis cache"
            >
              {#if rescanning}
            ...
            {#each season.episodes as ep (ep.episode_number)}
              <div class="flex items-center gap-3 px-4 py-2.5 hover:bg-base-content/5 transition-colors border-b border-base-content/5 last:border-b-0 {selectedEps.has(ep.path) ? 'bg-primary/5' : ''}">
                ...
                <button
                  ...
                  title="Analyze file"
                  onclick={(e) => { e.stopPropagation(); analyzePath = { path: ep.path, sonarr_episode_file_id: ep.sonarr_episode_file_id }; }}
                >
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
{#if analyzePath}
  <AnalyzeModal
    ...
    sonarr_episode_file_id={analyzePath.sonarr_episode_file_id}
    media_type="episode"
    onclose={() => (analyzePath = null)}
    onretagged={rescanShow}
