...
<div class="space-y-6">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    <a href="/jobs?filter=running" class="card-glass rounded-box p-4 stat-card hover:ring-1 hover:ring-info/40 transition-all">
      <div class="text-xs text-base-content/85 uppercase tracking-wider mb-1">Running</div>
    ...
    <a href="/jobs?filter=pending" class="card-glass rounded-box p-4 stat-card hover:ring-1 hover:ring-warning/40 transition-all">
      <div class="text-xs text-base-content/85 uppercase tracking-wider mb-1">Pending</div>
    ...
    <a href="/jobs?filter=completed" class="card-glass rounded-box p-4 stat-card hover:ring-1 hover:ring-success/40 transition-all">
      <div class="text-xs text-base-content/85 uppercase tracking-wider mb-1">Completed</div>
    ...
    <a href="/jobs?filter=failed" class="card-glass rounded-box p-4 stat-card hover:ring-1 hover:ring-error/40 transition-all">
      <div class="text-xs text-base-content/85 uppercase tracking-wider mb-1">Failed</div>
  ...
  {#if config}
    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
      <div class="card-glass rounded-box p-3 flex items-center gap-2">
        ...
        <span class="text-base-content/95">Audio</span>
      </div>
      <div class="card-glass rounded-box p-3 flex items-center gap-2">
        <span class="relative flex h-2 w-2">
        ...
        <span class="text-base-content/95">Video</span>
      </div>
      <div class="card-glass rounded-box p-3 flex items-center gap-2">
        <span class="relative flex h-2 w-2">
  ...
  {#if activeJobs.length > 0}
    <section>
      <h2 class="text-sm font-semibold uppercase tracking-wider text-base-content mb-3 flex items-center gap-2">
  ...
  <!-- Recent activity -->
  <section>
    <h2 class="text-sm font-semibold uppercase tracking-wider text-base-content mb-3">Recent Activity</h2>
    {#if loading}
    ...
    {:else if recentJobs.length === 0}
      <div class="card-glass rounded-box p-8 text-center">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 mx-auto mb-3 text-base-content/75" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
          <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 0 1 0 3.75H5.625a1.875 1.875 0 0 1 0-3.75Z" />
        </svg>
        <p class="text-sm text-base-content/85">No recent activity</p>
        <p class="text-xs text-base-content/75 mt-1">Jobs will appear when Sonarr/Radarr sends webhooks</p>
      </div>
    {:else}
      <div class="space-y-2">
